...
export const Mybloodrqstcollg = () => {
  ...
  useEffect(() => {
    let fetchdata = async () => {
      let response = await axios.get(`https://lifelink-backend-evaw.onrender.com/hospital/vwcollgdetail`)
  ...
  let handleDistrict=async(event)=>
  {
    if(event.target.value){
      ...
      let response=await axios.get(`https://lifelink-backend-evaw.onrender.com/college/cllgdistrict/${event.target.value}`)
  ...
  //   setData({...data,})
        ...
        let handleSubmit=async (event)=>{
            ...
            console.log(data);
           let response=await axios.post('https://lifelink-backend-evaw.onrender.com/hospital/mybloodrqstcllg',{...data,userId:id})
           console.log(response);
...
<div class="mb-5">
  ...
  <div class="">
    ...
    <input onChange={handleChange} value={data.students} name="students" type="number" min="1" id="age" class="shadow-sm bg-red-200 border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" required />
  </div>
 </div>
